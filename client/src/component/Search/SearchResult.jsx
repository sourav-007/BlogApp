...
import { FaAngleDoubleRight, FaSort } from 'react-icons/fa';
...
function SearchResult() {
    ...
    const { getSortedPost, searchResults, isLoading, error } = useBlog()
    ...
    const closeDropdown = async (option) => {
        ...
        let sortValue = { timePeriod: option }

        if (option === 'Most views') {
            sortValue = { mostViews: true }
        }
    ...
    //         setSearchResults(JSON.parse(savedResults));  // Set the saved search results
    //     }
    // }, [setSearchResults]);
    ...
    return (
        <>
            <section className="container mx-auto px-4 py-8 mt-32 border-2 border-solid border-red-700">
...

                {
                    searchResults && searchResults.length > 0 ? (
                        ...
                        <p className="text-center font-bauhaus text-5xl text-primary-text">
                            No results found for "<span className='text-special' >{qry}</span>"
                        </p>
                    )
                }
                            ...
                            searchResults.map((post, index) => (
                                ...
                                >
                                    ...
                                    <div className="flex-grow p-2 bg-slate-50 rounded-b-xl">
                                        ...
                                        <h3 className="text-xl font-semibold text-primary-text mb-2">{post.title}</h3>
                                        <p className="text-muted-foreground text-gray-700 text-justify mb-2">{post.content.slice(0, 145) + '.....'}</p>
